...
const VehicleTab = () => {
  ...
  const handleSave = (vehicle) => {
    if (currentVehicle) {
      setVehicles(
        vehicles.map((v) => (v.id === currentVehicle.id ? { ...vehicle, id: currentVehicle.id } : v))
      );
    } else {
          ...
          element={
            <>
              <div className="tab-header">
                ...
                />
                <button onClick={handleAdd} className="add-button">+ Add New</button>
              </div>
                ...
                {filteredVehicles.map((vehicle) => (
                  <Link to={`/vehicle/${vehicle.id}`} key={vehicle.id} className="vehicle-card">
                    <div className="vehicle-photo-section">
                      {vehicle.img ? (
                        <img src={vehicle.img} alt={`${vehicle.model}'s photo`} className="vehicle-photo" />
                      ) : (
                    ...
                    <div className="vehicle-info">
                      ...
                      <p>Assigned Driver:<strong>{vehicle.driverAssigned}</strong></p>
